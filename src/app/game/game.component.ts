...
export class GameComponent implements OnInit {
  ...
  move(xDirection: number, yDirection: number) {
    ...
    let newSnakeHead = new SnakePart(
      currentSnakeHead!.x + xDirection,
      currentSnakeHead!.y + yDirection
    );
    if (
      newSnakeHead.x < 0 ||
      newSnakeHead.x > this.columns - 1 ||
      newSnakeHead.y < 0 ||
      newSnakeHead.y > this.rows - 1
    ) {
  ...
  checkForLoseCondition() {
    if (this.playerLost) {
      ...
      if (this.score > this.highScore) localStorage.setItem('HighScore',this.score.toString());
